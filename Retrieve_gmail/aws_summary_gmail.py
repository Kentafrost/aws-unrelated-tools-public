...
def main():
    ...
    # AWS mails list in 勉強用/AWS label in gmail
    aws_emails = []
    
    try:
        # Get labels to find AWS label ID
        labels_result = gmail_service.users().labels().list(userId='me').execute()
        labels = labels_result.get('labels', [])
        aws_label_id = None
        
        # Find AWS label ID (check for both main labels and nested labels)
        for label in labels:
        ...
        if not aws_label_id:
            # Try to find nested labels containing AWS
            for label in labels:
        ...
        if aws_label_id:
            # Get messages with AWS label
            aws_results = gmail_service.users().messages().list(
                userId='me', 
                labelIds=[aws_label_id],
                maxResults=3000 # if you want more messages, modify this
            ).execute()
